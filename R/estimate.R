# ============================================================
# iconic_estimate: Real-data estimation layer for the
...
#' @param se_method Character: \code{"delta"} (default),
#' \code{"bootstrap"}, or \code{"composite"}. When
...
.estimate_mediation_driver <- function(data, methods, alpha, n_cores = 1,
  ...
  # when se_method == "composite", replace NIE_p with the
...
# ============================================================
# Survival outcome estimation drivers
#
...
#' Total-effect survival estimation driver
#'
...
#' Mediation survival estimation driver
#'
...
.estimate_mediation_surv_driver <- function(data, methods, alpha,
  ...
  run_one <- function(m, method) {
    ...
    r <- switch(as.character(method),
      ...
      IV2SLS2 = fit_iv2sls_mediation2_surv(time, event, X, M_vec, G, gm,
