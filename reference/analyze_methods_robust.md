...
PGC (matrix bridge) to each tested outcome feature and returns tidy
per-feature results with a significance flag. Negative controls are
summarised over the full W panel for COCA, so restricting
`test_features` does not change its control summary. The matrix-bridge
PGC uses the full W panel (or a validity-screened subset if provided).
