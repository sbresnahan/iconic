...
Two-stage Mendelian-randomization mediation with a binary outcome stage
(2SPS), with optional **path-specific** negative-control (NC)
augmentation:
...
3.  Logistic / LPM: `y ~ X_hat + M_hat (+ W2) + covars` -\> NDE (coef on
    X_hat), beta_M (coef on M_hat).
