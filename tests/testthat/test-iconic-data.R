...
test_that("iconic_data handles matrix mediator", {
  ...
  d <- iconic_data(X = rnorm(50), Y = matrix(rnorm(50 * 3), 3, 50), M = M)
  expect_true(d$is_mediation)
...
test_that("iconic_data sets W1=W2=W when only W supplied", {
  ...
  # W1/W2 are scaled separately from W, so they match only up to
  # floating-point precision, not exactly.
  expect_equal(d$W1, d$W)
  expect_equal(d$W2, d$W)
})
...
test_that("iconic_data rejects n < 20", {
  expect_error(iconic_data(X = rnorm(10), Y = matrix(rnorm(10 * 3), 3, 10)),
               "At least 20 samples")
})

test_that("iconic_data rejects missing X", {
  expect_error(iconic_data(Y = matrix(rnorm(50 * 3), 3, 50)), "X.*required")
})

test_that("iconic_data rejects missing Y", {
  expect_error(iconic_data(X = rnorm(50)), "Y.*required")
})
...
test_that("iconic_data handles matrix X (column means)", {
  Zmat <- matrix(rnorm(50 * 3), 3, 50)
  d <- iconic_data(X = Zmat, Y = matrix(rnorm(50 * 3), 3, 50))
  expect_equal(d$n, 50)
  expect_length(d$X, 50)
})
...
test_that("iconic_data handles matrix G (extracts first column)", {
  ...
  # scale = FALSE so d$G is on the raw scale and matches Gmat[, 1] exactly.
  d <- iconic_data(X = rnorm(50), Y = matrix(rnorm(50 * 3), 3, 50), G = Gmat,
                   scale = FALSE)
